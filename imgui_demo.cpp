...
//static std::map<std::string, char *> bufmap;
static std::map<std::string,std::string> bufmap;
//bufmap.second.resize(20);
//static std::map<std::string, char[50]> bufmap;
...
void ImGui::ShowDemoWindow(bool* p_open)
{
 ...
 //code when New button is selected 
    if (ImGui::Button("New")) {
        ...
        //bufmap.insert(std::pair<std::string,char*>(word_note, "      "));
        //bufmap.insert(std::map<std::string, char[50]>(word_note, "      "));
    }
 ...
 // code when an item on the list is selected
        ...
        if (ImGui::ListBoxHeader("List new"))
        {
            ...
            {
                x.second.resize(30);
                static char* buf = const_cast<char *>(x.second.c_str());
                //static char* buf = const_cast<char *>(x.second);
                //x.second = buf;
                ...
                ImGui::Selectable(x.first.c_str(), true);
                {
                   ...
                   // ImGui::InputTextMultiline(x.first.c_str(), const_cast<char *>(x.second.c_str()), IM_ARRAYSIZE(const_cast<char *>(x.second.c_str())), ImVec2(-2.0f, ImGui::GetTextLineHeight() * 4));

                   char* output = const_cast<char *>(buf);
                   //char* output = const_cast<char *>(x.second);
                    if (ImGui::Button("Save")) {
                        takeinputtodisk(const_cast<char *>(x.first.c_str()),output);
                    }
